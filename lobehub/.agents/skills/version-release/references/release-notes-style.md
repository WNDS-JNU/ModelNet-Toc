...
CONTRIBUTOR_COUNT=$(git log "$PREV_TAG..origin/release/weekly-{YYYYMMDD}" --no-merges --pretty=format:'%an' \
  | sort -u \
  | grep -viE '^(lobehubbot|ModelNet Bot|renovate\[bot\])$' \
  | wc -l | tr -d ' ')
```
...
Use the result for `@handle`. Then classify each author per the `ModelNet team roster` below; community first, team after.
...
1. `# 🚀 ModelNet Release (<YYYYMMDD>)` + scope line
...
**ModelNet team roster** — anyone in this list is a team member; anyone not in this list is a community contributor:
