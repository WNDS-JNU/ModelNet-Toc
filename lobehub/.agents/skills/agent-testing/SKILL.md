...
name: agent-testing
description: >
  Agentic end-to-end testing for ModelNet: backend verification via the CLI,
...
```
Step -1: Plan approval  →  Step 0: Env + Auth  →  Step 1: Pick surface  →  Step 2: Run  →  Step 3: Structured report  →  Step 4: Publish to ModelNet
```
...
| `app-probe.sh`            | ModelNet app probes: `auth` / `route` / `ops` / `goto <path>` / `errors`      |
...
`app-probe.sh` is the ModelNet-specific fast path into app state — auth check,
...
## Step 4 — Publish to ModelNet (mandatory)

The local report under `.records/reports/` is the working artifact; the
**deliverable is the report opened in ModelNet**. Do not stop at local files —
...
├── cli/index.md        # backend verification via the ModelNet CLI
