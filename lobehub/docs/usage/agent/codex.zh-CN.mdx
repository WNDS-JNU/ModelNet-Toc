---
title: Codex
description: 在 ModelNet 中委派 OpenAI Codex —— 通过桌面应用与 Codex CLI 对话，实时查看文件变更、待办与命令输出，并跨轮次恢复会话。
tags:
  - ModelNet
  - Codex
  - 编程助理
...
Codex 是 OpenAI 推出的编程助理，能在终端中编辑文件、运行命令、提交改动。在 ModelNet 中，你可以通过桌面应用委派 Codex —— 保留熟悉的对话界面，让 Codex 在本地完成实际工作，并完整访问你的项目。

发送一条提示，Codex 会打开文件、修改代码、运行测试，并把过程反馈给你。文件变更、待办列表、命令输出会随着助理推进实时进入聊天；会话能跨轮次恢复，一项长任务可以分布在多条消息中持续推进。

## 什么是 ModelNet 中的 Codex？

它是 ModelNet 对话界面与本地 Codex CLI 之间的桥梁。ModelNet 在本地以子进程形式启动 Codex CLI，把它的事件流接入聊天会话，并将 Codex 的工具输出 —— 文件变更、待办列表、命令执行 —— 渲染为一等公民的聊天块。你用自然语言指挥助理，Codex 在本地用你的环境、凭据与项目上下文执行。

## 使用条件

- **ModelNet 桌面应用** —— Codex 助理只在桌面版可用，Web 端无法启动本地进程。
- **已安装 Codex CLI** —— `codex` 命令需要在你的 `PATH` 中可用。
- **已登录** —— 在 ModelNet 调用前，需在终端中先运行一次 `codex` 完成认证。
...
若 ModelNet 未能检测到 CLI，会弹出**安装 Codex CLI** 引导，并提供**打开系统工具**按钮 —— 安装完成后点击即可重新检测。

## 在 ModelNet 中添加 Codex

当 ModelNet 检测到本机已安装 Codex CLI，首页会出现一张标记为「编程助理」的**添加 Codex** 推荐卡片，点击即可一步创建 Codex 助理。
...
如果切换目录后，已保存的 Codex 线程无法安全恢复，ModelNet 会提示：**「已保存的 Codex 线程无法安全恢复，已为该话题开启新的会话。」**

## 聊天中会渲染什么

ModelNet 不会把 Codex 的工具调用渲染成原始 JSON，而是用专用区块呈现：
...
- **仅桌面端** —— Codex 助理只在 ModelNet 桌面应用中可用，Web 端无法启动 CLI。
- **每台机器一次登录** —— Codex 与全局 CLI 共享认证。终端里 `codex` 能用，ModelNet 里就能用。
- **绑定工作目录** —— 会话不会跨文件夹或机器跟随你。

## 使用技巧

- **先在终端中运行一次 `codex`** —— 登录在 CLI 层面完成，不在 ModelNet 里。
