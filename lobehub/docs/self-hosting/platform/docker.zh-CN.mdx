---
title: 使用 Docker 部署 ModelNet 数据库
description: 详细步骤教你如何在 Docker 中部署 ModelNet 服务端数据库。
tags:
  - Docker
  - ModelNet
  - 数据库部署
...
<Callout type="info">
  本文已经假定你了解了 ModelNet
  的部署基本原理和流程，因此只包含核心环境变量配置的内容。如果你还不了解 ModelNet
...
以下是在 Linux 服务器上部署 ModelNet 的流程：
...
<Steps>
  ...
  上述指令会创建一个名为 `my-postgres`，并且网络为 `pg` 的 Postgres 实例，其中 `paradedb/paradedb:latest-pg17` 是一个 Postgres 17 的镜像，且默认安装并启用了 `pgvector` 和 `pg_search` 插件。

  <Callout type="info">
    ParadeDB 镜像包含了 pgvector（向量搜索）和 pg\_search（全文搜索）插件，是 ModelNet 实现 RAG 和知识库搜索的重要构件。
  </Callout>
...
ModelNet 也支持直接在本地的 Mac/Windows 本地使用。
