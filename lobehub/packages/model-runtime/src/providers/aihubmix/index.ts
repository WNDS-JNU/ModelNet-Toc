...
/**
 * Maps AiHubMix `types` field values to ModelNet AiModelType.
 ...
 * ModelNet's AiModelType and are filtered out before model list processing to
...
/**
 * AiHubMix `types` values that have no corresponding ModelNet AiModelType.
...
/**
 * Map AiHubMix full-catalog API response fields to ModelNet model card fields.
...
const mapAiHubMixModel = (m: any): { [key: string]: any; id: string } => {
  ...
  return {
    ...
    // `structured_outputs` has no corresponding ModelNet model card field and is intentionally omitted.
    ...(featureSet && {
