...
```bash
modelnet verify submit --operation "$LOBE_OPERATION_ID" --item "$CHECK_ITEM_ID" --type gif \
  --file ./proof/flow.gif --by agent-browser \
  --desc "Response streams in token-by-token after send"

modelnet verify submit --operation "$LOBE_OPERATION_ID" --item "$CHECK_ITEM_ID" --type video \
