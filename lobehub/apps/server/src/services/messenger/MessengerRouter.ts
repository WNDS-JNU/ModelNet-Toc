...
import { agents, type MessengerAccountLinkItem } from '@/database/schemas';
 ...
 *
 * Account model: each `(ModelNet user, platform, tenant_id)` triple has at
 * most one row in `messenger_account_links`, so a single ModelNet user can
...
export class MessengerRouter {
  ...
  private registerHandlers(
    ...
    // listing reviewers also test the `/invite @ModelNet` entry point, so the
  ...
  private buildCommands(): MessengerCommand[] {
    return [
      {
        description: 'Bind your account to ModelNet',
        handler: async (ctx) => {
          ...
          if (ctx.link) {
            await ctx.reply(
              'Your account is already linked to ModelNet. Send /agents to switch the active agent, or /new to start a fresh conversation.',
            );
            return;
          }
          ...
          if (!ctx.isDM && !canEphemeralInChannel) {
            await ctx.reply('Check your DM with ModelNet for the link button.');
          }
        },
        name: 'start',
      },
      ...
      },
      {
        ...
        handler: async (ctx) => {
          ...
          if (!ctx.thread) {
            ...
            await ctx.reply('Open your direct message with the ModelNet bot and send `/new` there.');
            return;
          }
      ...
      },
      {
        ...
        handler: async (ctx) => {
          ...
          if (!ctx.thread) {
            await ctx.reply(
              'Open your direct message with the ModelNet bot and send `/stop` there.',
            );
            return;
          }
      ...
      {
        description: 'Send feedback directly to the ModelNet team (no AI reply)',
        ...
        handler: async (ctx) => {
          // Feedback is tied to a ModelNet account so the team can follow up;
  ...
  private async runAgentsCommand(ctx: MessengerCommandContext): Promise<void> {
    ...
    if (userAgents.length === 0) {
      await ctx.reply('You have no agents yet. Create one in ModelNet, then come back to /agents.');
      return;
    }
   ...
   * which ModelNet context a conversation runs in is the active agent's scope;
  ...
  private scopeSwitchedText(scopeName: string, defaultAgentTitle?: string): string {
    return defaultAgentTitle
      ...
      : `Switched to ${scopeName}. No agents here yet — create one in ModelNet, then /agents.`;
  }
  ...
  private async handleAppHomeOpened(
    ...
    try {
      ...
      const text = activeAgentName
        ? `Welcome to ModelNet! Your active agent is *${activeAgentName}*. Send a message to chat, or use \`/agents\` to switch.`
        : 'Welcome to ModelNet! Send `/agents` to pick an active agent and start chatting.';
   ...
   * itself joins a channel (via `/invite @ModelNet` or being added through the
  ...
  private async handleChannelJoin(
    ...
    const text = [
      ":wave: Hi, I'm *ModelNet* — your AI agent on Slack.",
      '',
      '• Mention me with `@ModelNet <your question>` to chat in this channel.',
      '• First time? Send me a *direct message* to link your ModelNet account.',
  ...
  private async dispatchToAgent(
    ...
    // ModelNet user; the dispatch only fires for the linked sender. So
