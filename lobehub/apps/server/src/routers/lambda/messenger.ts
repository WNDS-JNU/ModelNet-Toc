 ...
 * which ModelNet context a conversation runs in is derived from the *active
...
export const messengerRouter = router({
   ...
   * a ModelNet account — the page uses it to warn the user before they create
  ...
  peekLinkToken: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return {
        // Set when the IM identity is already linked to some ModelNet account.
  ...
  confirmLink: messengerProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // already bound to a different ModelNet account. The DB unique index
      ...
      try {
        ...
        // Race backstop: the IM identity got bound to another ModelNet user
   ...
   * inbox/ModelNet agent — historical inbox sessions get migrated with
   * `virtual=true`, so a plain virtual filter would hide ModelNet even though
   ...
   * inbox agent resolves to `"ModelNet"` + default avatar; everything else falls
  ...
  listAgentsForBinding: messengerProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (workspaceId) {
        ...
        const userWorkspaces = await new WorkspaceModel(
          ctx.serverDB,
          ctx.userId,
        ).listUserWorkspaces();
      ...
      const mapped = rows
        ...
        .map((row) => ({
          ...
          id: row.id,
          slug: row.slug,
          title: row.title || (row.slug === INBOX_SESSION_ID ? 'ModelNet' : null),
        }));
   ...
   * plus every workspace they belong to.
   */
  ...
  /**
   * List the Slack workspaces this ModelNet user has installed the bot into.
  ...
  uninstallInstallation: messengerWriteProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // it. Workspace admins who installed via a different ModelNet account
