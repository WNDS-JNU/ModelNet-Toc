#!/usr/bin/env python3
"""Generate ModelNet ModelNet provider settings from the ModelNet capability registry."""
...
def render_env_content(api_key: str, entries: list[str]) -> str:
    return "\n".join(
        [
            "# Generated by scripts/sync_modelnet_app.py from ModelNet capability registry.",
            "MODELNET_API_KEY=" + api_key,
            "MODELNET_PROXY_URL=http://modelnet-router:8000/v1",
            "MODELNET_MODEL_LIST=-all," + ",".join(entries),
            "",
        ]
    )
...
def main() -> int:
    ...
    api_key = (
        os.environ.get("MODELNET_BACKEND_API_KEY")
        or read_env_value(args.env_file, "MODELNET_BACKEND_API_KEY")
        or "none"
    )
