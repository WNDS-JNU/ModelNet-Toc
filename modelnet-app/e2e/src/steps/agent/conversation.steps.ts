...
Given('用户进入 ModelNet 对话页面', { timeout: 30_000 }, async function (this: CustomWorld) {
  ...
  console.log('   📍 Step: 直接进入 ModelNet 对话路由...');
...
Given('用户已发送消息 {string}', async function (this: CustomWorld, message: string) {
  ...
  // Assistant messages are left-aligned .message-wrapper elements that contain "ModelNet" title
  ...
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('text=ModelNet'),
  });
...
Then('回复内容应该可见', async function (this: CustomWorld) {
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('.message-header', { hasText: /ModelNet|AI/ }),
  });
