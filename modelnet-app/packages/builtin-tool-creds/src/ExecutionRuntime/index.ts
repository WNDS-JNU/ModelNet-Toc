...
export class CredsExecutionRuntime {
  ...
  async connectComposioService(
    ...
    return {
      content: `To connect ${serverType.label}, please use the ModelNet app UI to initiate the Composio OAuth flow. Server-side execution cannot open OAuth popups. Go to Settings or the onboarding page to connect ${serverType.label}.`,
      state: {
        connected: false,
        identifier: service,
  ...
  async initiateOAuthConnect(
    ...
    try {
      ...
      const appUrl = (process.env.APP_URL || 'http://123.56.135.150').replace(/\/+$/, '');
