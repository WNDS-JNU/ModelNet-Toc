...
const normalizeEndpoint = (
  ...
  try {
    ...
    // ModelNet's Google runtime accepts a host root and lets the SDK append its
...
export const resolveHeterogeneousProviderBinding = ({
  ...
  if (
    ...
    // Kimi only knows the selected wire protocol, not ModelNet's provider id.
    // Without an explicit endpoint, its SDK would silently use OpenAI or
    // Anthropic instead of a third-party provider whose URL ModelNet normally
...
export const formatHeterogeneousProviderBindingError = (
  ...
  switch (error.code) {
    case 'agentUnsupported': {
      return `${error.agentType} does not support ModelNet Provider binding.`;
    }
    case 'configMissing': {
