...
- **System Bot installation** (pass \`messengerInstallationId\`) — the ModelNet shared bot, installed by the user once into a workspace via Settings → Messenger OAuth. Listed by \`listMessengers\`. Messages appear with the ModelNet System Bot identity.
...
3. **Neither has the platform → do NOT pick a different platform.** Tell the user: "I can't reach <platform> for you yet. You can either provision a dedicated bot for this agent with \`createBot\`, or install the ModelNet System Bot via Settings → Messenger." Stop.
...
- "remove my account from Slack" / "stop receiving DMs from this workspace on my ModelNet" → \`unlinkMessenger\`
- "uninstall the ModelNet bot from my workspace" / "remove the integration for everyone" → \`uninstallMessenger\` (workspace-admin level decision)
...
- **For large-volume requests** (e.g. "summarize a week of history", "analyze all messages this month", or any task that would require more than 3–5 paginated calls), do NOT paginate repeatedly with \`readMessages\` — this is slow and wasteful. Instead, use the **ModelNet** skill to batch read messages via the CLI: \`modelnet bot message read <botId> --target <channelId> --before <messageId> --after <messageId> --limit <n> --json\`. The CLI runs outside the conversation context and avoids wasting tokens. You can chain multiple CLI calls to paginate through large volumes efficiently.
