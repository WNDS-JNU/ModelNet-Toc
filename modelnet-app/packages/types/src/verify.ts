...
/**
 * The ModelNet conversation an ingested report was authored in. Lets the report
...
export interface VerifyRunMetadata {
  ...
  maxRepairRounds?: number;
  /**
   * Where this report came from — the ModelNet conversation whose agent produced
   * it. Set when the harness runs inside a ModelNet-spawned agent (the runtime
 ...
 * text, `documentId` for a ModelNet document, or `fileId` for a stored artifact.
 */
export interface VerifyEvidence {
  ...
  /** ModelNet document evidence — FK to `documents`. */
  documentId?: string | null;
