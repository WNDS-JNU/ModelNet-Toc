 ...
 * interceptor cannot tell an internal ModelNet URL from a third-party one by origin
 ...
 * originates from the Market service's own OAuth token, NOT from the user's ModelNet
 * session.  responseMeta checks this to suppress the X-Auth-Required header so the
 * desktop "re-login to ModelNet" modal is NOT shown; the Market OAuth flow handles it
