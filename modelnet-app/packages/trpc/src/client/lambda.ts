            ...
            switch (status) {
              case 401: {
                if (isMarketApi) {
                  ...
                  // Don't trigger ModelNet logout for market auth issues
                  const { getUserStoreState } = await import('@/store/user/store');
                  // Without a ModelNet session a market.* 401 is not a Market auth
                  ...
                  // Non-market 401: handle as before (ModelNet session expired)
                  const now = Date.now();
