 ...
 * 2. Adding X-Auth-Required header for ModelNet session UNAUTHORIZED errors
 *
 * The X-Auth-Required header allows the desktop app (BackendProxyProtocolManager)
 * to distinguish between real ModelNet session failures (e.g., token expired)
...
export function createResponseMeta({ ctx, errors }: ResponseMetaParams): {
  ...
  // Only set X-Auth-Required for ModelNet session failures, not for Market OAuth failures.
