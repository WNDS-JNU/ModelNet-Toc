...
export const createAsyncRouteContext = async (request: NextRequest): Promise<AsyncContext> => {
  ...
  log('ModelNet auth header present: %s', !!lobeChatAuthorization);
  ...
  if (!lobeChatAuthorization) {
    log('No ModelNet authorization header found');
    throw new Error('No ModelNet authorization header found');
  }
  ...
  try {
    ...
    log('Decrypting ModelNet authorization');
