 ...
 * different ModelNet user. Callers (e.g. the messenger router) should surface
...
export class MessengerAccountLinkConflictError extends Error {
  ...
  constructor(existingUserId: string, message?: string) {
    super(message ?? 'IM identity is already linked to another ModelNet user');
...
/**
 * Thrown when the same ModelNet user already has a different IM identity bound
...
export class MessengerAccountLinkModel {
  ...
  private throwApplicationClaimConflict = async (
    ...
    if (claimed && claimed.userId !== this.userId) {
      throw new MessengerAccountLinkConflictError(
        claimed.userId,
        'Credential application is already linked to another ModelNet user',
      );
    }
   ...
   * ModelNet user; we throw `MessengerAccountLinkConflictError` instead.
   *
