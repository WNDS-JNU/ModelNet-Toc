...
/**
 * Maps a ModelNet user to a single IM account per platform (e.g. one Telegram
 * account ↔ one ModelNet user). The active agent for that IM session is
...
export const messengerAccountLinks = pgTable(
  ...
  (t) => [
    // One IM account per (platform, tenant) binds to exactly one ModelNet user.
    // The tenant column lets the same Slack user id under two workspaces
    // bind to different ModelNet users — without it, the second workspace
    ...
    // One ModelNet user has at most one IM account per (platform, tenant) —
