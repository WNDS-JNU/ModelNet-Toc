 ...
 * skill (macOS scripts + local report dirs + ModelNet-specific probes), this one
 * depends only on the `modelnet` CLI and `agent-browser`, so any external builder
 * (Claude Code / Codex) can run it from a task's working directory: discover the
 * plan → pick a surface → capture evidence per criterion → `modelnet verify
 ...
 * ModelNet-only coupling generalized away.
 *
...
export const VerifySkill: BuiltinSkill = {
  ...
  content,
  description:
    'Self-evidence for task delivery verification — discover the verify plan, pick the right surface (CLI / web / desktop), drive it with agent-browser, get past auth, capture portable evidence per criterion, and submit each with `modelnet verify submit` so the delivery is judged on real proof.',
