...
description: >
  ...
  ambient ids, and never depends on running inside a ModelNet conversation.
---
...
- **Attaching to something specific?** Pass `--subject` (`task:<id>`,
  ...
  `modelnet acceptance run ingest` attaches the round itself when it can, and creates a
...
```bash
modelnet verify plan state "$OPERATION_ID" --json
```
...
modelnet acceptance run result submit --operation "$OPERATION_ID" --item "$CHECK_ITEM_ID" \
...
```bash
modelnet acceptance run result list --operation "$OPERATION_ID" --json # checkItemId → checkResultId
