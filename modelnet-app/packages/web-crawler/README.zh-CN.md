...
ModelNet 内置的网页抓取模块，用于智能提取网页内容并转换为 Markdown 格式。

## 📝 简介

`@lobechat/web-crawler` 是 ModelNet 的核心组件，负责网页内容的智能抓取与处理。它能够从各类网页中提取有价值的内容，过滤掉干扰元素，并生成结构化的 Markdown 文本。
...
这是 ModelNet 的内部模块（`"private": true`），专为 ModelNet 设计，不作为独立包发布使用。
