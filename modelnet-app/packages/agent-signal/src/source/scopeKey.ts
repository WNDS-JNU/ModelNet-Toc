...
/**
 * Bot-thread identity used to route source events that originate outside a ModelNet topic.
 */
...
export interface AgentSignalProducerScopeInput {
  ...
  taskId?: string;
  /** Optional ModelNet topic identifier. Takes precedence over task and bot-thread metadata. */
  topicId?: string;
...
/**
 * Topic identity used when an AgentSignal chain is scoped to one ModelNet topic.
 */
export interface AgentSignalTopicScopeKeyInput {
  /** Stable ModelNet topic identifier. */
  topicId: string;
}
