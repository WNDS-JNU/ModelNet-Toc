...
export interface ConnectorCatalogAvailability {
  ...
  composio: boolean;
  /** Whether ModelNet Market connectors may be selected. */
  lobehub: boolean;
}
 ...
 * - ModelNet and Composio catalogs may contain the same identifier
 *
 * Expects:
 * - Explicitly source-qualified flows use their declared source instead
 * - ModelNet owns collisions even when its connector integration is unavailable
 *
 * Returns:
 ...
 *
 * Returns:
 * - ModelNet connectors first, followed by non-conflicting Composio connectors
 */
