# PROJECT.md — agent-testing adapter for ModelNet

This is the ModelNet adapter for the generic `agent-testing` skill. The skill is
project-agnostic; every ModelNet-specific command, port, service, and probe lives
here. The skill reads this file — it never guesses ModelNet's commands.
...
ModelNet is a chat/agent product with a Next.js server, a Vite + React SPA, an
...
live in a separate ModelNet project skill, `agent-testing-bot`
...
`.agents/acceptance/scripts/app-probe.sh` is the ModelNet fast path into app state —
...
Deeper ModelNet-specific notes kept alongside the moved scripts:
...
- `.agents/acceptance/references/probe-field-notes.md` — historical, detailed
  ModelNet probe incidents and their original cross-reference ids.
...
`.agents/acceptance/probe-mock-patterns.md`) hold the ModelNet-specific probe/mock and
