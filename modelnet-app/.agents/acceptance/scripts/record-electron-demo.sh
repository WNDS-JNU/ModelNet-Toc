...
get_window_and_screen_info() {
  ...
  swift -e '
    ...
    for w in windowList {
      ...
      if (owner == "Electron" || owner == "ModelNet") && layer == 0 && name == "ModelNet" && ww > 200 && wh > 200 {
...
builtin_demo() {
  ...
  # Try ModelNet first, then fall back to any agent link in the sidebar
  agent_ref=$(echo "$snapshot" | grep -oE 'link "ModelNet" \[ref=e[0-9]+\]' | grep -oE 'e[0-9]+' || true)
  if [ -z "$agent_ref" ]; then
