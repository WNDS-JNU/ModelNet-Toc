# ModelNet Probe & Mock Guide

This is the project-layer entry point for ModelNet acceptance probes. Read it
together with the agent-testing skill's generic `references/probe-mock-patterns.md`.
Product-independent rules belong upstream; ModelNet routes, stores, services, env
...
consecutive ticks — ModelNet's boot routinely shows a single 0.6–1.1s blocking task, so a
...
- [Probe field notes](./references/probe-field-notes.md) — all historical
  ModelNet findings, original identifiers, commands, and failure analysis.
