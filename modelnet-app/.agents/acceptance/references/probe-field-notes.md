# Probe Field Notes — ModelNet historical detail

> Historical, ModelNet-specific field notes preserved for mechanism detail and old
...
- **Situation**: after `electron-dev.sh start <id>`, `app-probe.sh auth` returns `isSignedIn:false`,
  `#root` has providers but `innerText.length === 0`, and the screenshot is just the ModelNet splash. The
...
- **Situation**: `electron-dev.sh start <id>` seeds userData from the golden dev profile,
  but the app renders an empty `#root` (innerText length 0, only the ModelNet watermark) and
...
- **Doesn't work**: pointing `LOBE_GOLDEN_PROFILE` at the packaged app's profile
  (`~/Library/Application Support/ModelNet`) — the pool instance's startup refresh will rotate
...
- **Situation**: driving ModelNet UI (ActionIcon, dropdown items) via `Runtime.evaluate`; `el.click()`
