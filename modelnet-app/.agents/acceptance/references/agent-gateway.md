# ModelNet gateway streaming + tab-switch test harness
...
works for any ModelNet streaming session.
...
- **Never identify tabs by innerText.** The active tab's text embeds a
  ` · <agent name>` suffix, so a search like `'ModelNet Growth'` matches the
  active tab when the active agent happens to be ModelNet Growth — and you
