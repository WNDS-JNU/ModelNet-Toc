...
Keep this file at the level of durable ModelNet product and environment invariants.
...
lose their tool output, render it under the wrong Agent, or invoke ModelNet for
the final answer.
...
**Wrong approach:** attach to port 9222 and assume it belongs to the ModelNet dev
Electron instance.

**Why it fails:** another Electron project can own the universal default port, while
the user's ModelNet instance may expose no debugging port at all.

**Correct approach:** verify the owning process path and probe a ModelNet-specific
...
**Same failure, second shape — the instance is ModelNet, but a different worktree.**
...
**Why it fails:** the property lives in the returned bytes, not in the request. ModelNet's
...
`curl` as the health signal for an unattended ModelNet soak.
