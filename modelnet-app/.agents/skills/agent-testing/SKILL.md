...
description: >
  ...
  confirmed evidence, and publishes a structured report to ModelNet Acceptance.
...
### Step 6 — Publish to ModelNet Acceptance (mandatory)
...
An external project normally has no ModelNet Agent Operation, and that is valid.
Do not ask the user for an Operation ID, invent one, or pass `--operation` merely
because the report is being published to ModelNet.
...
- `--operation <id>` means “this verification session evaluates this existing
  ModelNet Agent Run.” Use it only when the user explicitly targets a real
  operation that exists in the current ModelNet workspace.
...
2. Otherwise, when the verification continues work discussed and implemented in
   the current ModelNet conversation, use its `topic:<id>`. This is the default for
...
When the harness runs inside a relevant ModelNet topic, `acceptance run ingest`
...
Outside a ModelNet topic, an explicit subject is required and publishing without
one fails:
