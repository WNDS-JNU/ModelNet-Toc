...
6. **Publish** (SKILL.md Step 6) — upload the finished session so it's viewable on
   ModelNet Acceptance, not just on disk. **Publish to PRODUCTION defaults with the
...
`--operation` only to link the report to a real, existing ModelNet Agent Run under
...
Inside a relevant ModelNet conversation, both subject fields may be omitted because
