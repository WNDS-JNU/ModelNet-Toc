...
export class MarketService {
  ...
  /**
   * Search for skills in the ModelNet Market
   */
  ...
  async executeLobehubSkill(params: LobehubSkillExecuteParams): Promise<LobehubSkillExecuteResult> {
    ...
    try {
      const timeoutPromise = new Promise<never>((_, reject) => {
        timeout = setTimeout(() => {
          const error = new Error(`ModelNet Skill execution timed out after ${timeoutMs}ms`);
      ...
      if (!response.success) {
        ...
        const message = responseError?.message || dataMessage || 'ModelNet Skill call failed';
  ...
  /**
   * Fetch ModelNet Skills manifests from Market API
      ...
      for (const connection of connections) {
        try {
          ...
          const manifest: LobeToolManifest = {
            ...
            meta: {
              avatar: icon || '🔗',
              description: `ModelNet Skill: ${providerLabel}`,
