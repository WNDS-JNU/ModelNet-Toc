...
const SYSTEM_STRINGS: Partial<Record<BotReplyLocale, SystemStrings>> = {
  'en-US': {
    ...
    cmdFeedbackSubmitted: 'Thanks — your feedback has been sent to the ModelNet team.',
    cmdFeedbackSubmittedWithLink: (issueUrl) =>
      `Thanks — your feedback has been sent to the ModelNet team. Tracked at: ${issueUrl}`,
    cmdFeedbackUsage:
      'Usage: `/feedback <your message>` — sends feedback directly to the ModelNet team (no AI reply).',
    ...
    errorInsufficientCredits:
      "**Not enough credits.**\nYour remaining credits can't cover this model's estimated cost. Please top up credits or upgrade your plan on the ModelNet website, or switch to a less expensive model in the agent's settings.",
  ...
  'zh-CN': {
    ...
    cmdFeedbackSubmitted: '已收到，感谢反馈，已转交 ModelNet 团队。',
    cmdFeedbackSubmittedWithLink: (issueUrl) =>
      `已收到，感谢反馈，已转交 ModelNet 团队。跟踪链接：${issueUrl}`,
    cmdFeedbackUsage:
      '用法：`/feedback <你的反馈内容>` —— 反馈会直达 ModelNet 团队，不会触发 AI 回复。',
    ...
    errorInsufficientCredits:
      '**积分余额不足**\n剩余积分不足以覆盖本次模型调用的预估费用。请前往 ModelNet 网页端充值积分或升级订阅计划，或在 Agent 设置中切换到费用更低的模型。',
    errorInvalidProviderAPIKey:
