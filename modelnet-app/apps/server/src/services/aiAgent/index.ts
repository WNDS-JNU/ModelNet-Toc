...
export class AiAgentService {
  ...
  private async execAgentWithReservation(
    ...
    if (isHeteroAgent) {
      ...
      // standard 'github' key (ModelNet OAuth connector default); agent config can
    ...
    if (params.disableTools) {
      ...
      // 5c. Fetch ModelNet Skills manifests
      try {
        const marketService = await this.getMarketService();
      ...
      const pluginIds = [
        ...new Set([
          ...
          // Include ModelNet Skills and Composio tools so they are passed to generateToolsDetailed
          ...lobehubSkillManifests.map((m) => m.identifier),
          ...composioManifests.map((m) => m.identifier),
      ...
      // installed plugin, a ModelNet Skill, or a Composio manifest declaring
