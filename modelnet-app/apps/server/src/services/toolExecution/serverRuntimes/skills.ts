...
const LEGACY_FALLBACK_NOTE =
  "Note: the user's device client is outdated and does not support on-device skill execution, so this command ran in the cloud sandbox instead. Tell the user to update their ModelNet app to run skills on their device.";
        ...
        if (!prepared.success || !prepared.extractedDir) {
          ...
          return fail(
            `Failed to prepare skill "${archive.name}" on the user's device: ${prepared.error ?? 'unknown error'}. ` +
              'Do not retry elsewhere — report this to the user (their ModelNet app may need an update).',
          );
        }
