    ...
    try {
      ...
      if (isDeviceOnlyMcp && deviceGateway.isConfigured) {
        ...
        if (!tunnelTarget) {
          ...
          const message = `MCP server '${identifier}' only your own machine can reach (stdio or local network). No online device was found to run it — open the ModelNet desktop app on the machine that hosts this MCP server, then retry.`;
          return {
            content: message,
