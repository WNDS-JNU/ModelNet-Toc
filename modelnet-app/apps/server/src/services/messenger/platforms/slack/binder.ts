...
export class MessengerSlackBinder implements MessengerPlatformBinder {
  ...
  async handleUnlinkedMessage(ctx: UnlinkedMessageContext): Promise<void> {
    ...
    try {
      ...
      const errorText = 'ModelNet is temporarily unavailable. Please try again in a moment.';
    ...
    if (ctx.channelMentionThreadId) {
      const [, channelId, threadTs] = ctx.channelMentionThreadId.split(':');
      const text =
        "Hi, I'm ModelNet — your AI agent on Slack.\n" +
        `Link your ModelNet account to start chatting: <${verifyUrl}|click here>`;
    ...
    const intro =
      "Hi, I'm ModelNet — your AI agent on Slack.\n" + 'To start, link your ModelNet account.';
  ...
  async notifyLinkSuccess(params: {
    ...
    const headline =
      ':white_check_mark: Linked successfully! Your ModelNet account is now connected.';
   ...
   * `@ModelNet` in a public channel — we want them to get the verify-im URL
