...
export function generateRootManPage(program: Command, version: string) {
  ...
  const lines = [
    ...
    `.TH MODELNET 1 "" "${escapeRoff(`@modelnet/cli ${version}`)}" "User Commands"`,
    '.SH NAME',
    `modelnet \\- ${escapeRoff(program.description() || 'ModelNet CLI')}`,
    '.SH SYNOPSIS',
    ...formatSynopsisLines(),
    '.SH DESCRIPTION',
    escapeRoff(
      `${program.name()} is the command-line interface for ModelNet. It provides authentication, device gateway connectivity, content generation, resource search, and management commands for agents, files, models, providers, plugins, knowledge bases, threads, topics, and related resources.`,
    ),
    '.PP',
