...
export const getProtocolScheme = (): string => {
  const appName = String(app.name || app.getName?.() || '').toLowerCase();
  const appPath = app.getPath('exe').toLowerCase();
  const prefix = appName.includes('modelnet') || appPath.includes('modelnet')
    ? 'modelnet'
    : 'lobehub';

  if (appName.includes('nightly') || appPath.includes('nightly'))
    return `${prefix}-nightly`;
  if (appName.includes('beta') || appPath.includes('beta')) return `${prefix}-beta`;
  if (appName.includes('dev') || appPath.includes('dev')) return `${prefix}-dev`;
...
export function generateRFCProtocolUrl(params: {
  ...
  /** MCP Schema object */
  schema: McpSchema;
  /** Protocol scheme (default: current ModelNet channel) */
  scheme?: string;
}): string {
  const { id, schema, marketId, scheme = getProtocolScheme() } = params;
 ...
 *     name: 'EdgeOne MCP',
 *     author: 'Higress Team',
 *     description: 'EdgeOne API integration for ModelNet',
 *     version: '1.0.0',
 *     config: {
 *       type: 'stdio',
 *       command: 'npx',
