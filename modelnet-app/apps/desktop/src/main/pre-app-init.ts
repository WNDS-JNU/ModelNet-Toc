...
if (electronIs.dev()) {
  ...
  // absolute path via MODELNET_DESKTOP_USER_DATA_DIR for multi-instance testing.
  app.setName('modelnet-desktop-dev');
  const userDataOverride =
    process.env.MODELNET_DESKTOP_USER_DATA_DIR || process.env.LOBE_DESKTOP_USER_DATA_DIR;
  app.setPath(
    'userData',
    userDataOverride || path.join(app.getPath('appData'), 'modelnet-desktop-dev'),
  );
}
