 ...
 * Tells the agent binary how to push results back to the ModelNet chat UI via `lh notify`.
...
function buildNotifyProtocol(lhPath: string, topicId: string): string {
  return (
    `## Context: This task was dispatched by ModelNet\n\n` +
    `This conversation / task was sent to you by the **ModelNet platform** on behalf of a user. You are running as a background agent; the user is waiting for your response inside the ModelNet chat interface.\n\n` +
...
export default class GatewayConnectionCtr extends ControllerModule {
  ...
  private async runHeteroTask(args: {
    ...
    if (agentType === 'openclaw') {
      ...
      // back to the ModelNet UI — even if the previous turn failed and the session
