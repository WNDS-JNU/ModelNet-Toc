...
interface StartSessionParams {
  ...
  initialModel?: string;
  /** Credential-free ModelNet Provider reference. Desktop main resolves its secrets. */
...
interface AgentSession {
  ...
  /** Present iff the session runs on the server-default (ModelNet) binding. */
...
export default class HeterogeneousAgentCtr {
    ...
    if (!status || status.available) {
      if (
        ...
        return {
          ...
          command,
          message: `Kimi Code 0.6.0 or newer is required to use a ModelNet provider. Installed version: ${status.version}.`,
          workingDirectory,
        };
      }
  ...
  private async sendPromptImpl(params: SendPromptParams): Promise<void> {
    ...
    // from the turns ModelNet still holds turns a hard
  ...
  spawnLhHeteroExec(params: {
    ...
    // the misleading `spawn ModelNet.exe ENOENT`.
