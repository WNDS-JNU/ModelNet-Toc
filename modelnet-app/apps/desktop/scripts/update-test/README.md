...
3. 切换后应用会自动检查对应渠道的更新
4. 查看日志确认 feed URL 切换正确：`tail -f ~/Library/Logs/modelnet-desktop-dev/main.log`
...
```bash
tail -f ~/Library/Logs/modelnet-desktop-dev/main.log | grep -E 'Switching|Configuring|channel|checking'
```
...
- 查看日志确认 `configureUpdateProvider` 被调用：`grep 'Configuring generic' ~/Library/Logs/modelnet-desktop-dev/main.log`
