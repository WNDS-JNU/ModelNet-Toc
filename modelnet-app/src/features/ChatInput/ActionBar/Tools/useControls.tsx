...
export const useControls = ({ closeDropdown }: { closeDropdown?: () => void } = {}) => {
  ...
  // Load user's ModelNet Skill connections via SWR
  ...
  // ModelNet Skill Provider list items - only show installed or recommended
  ...
  // Builtin tool list items (excluding Composio and ModelNet Skill)
  ...
  // Skills list items (including ModelNet Skill and Composio)
  // Connected items listed first, deduplicated by key (ModelNet takes priority)
  const skillItems = useMemo(() => {
    // Deduplicate by key - ModelNet items take priority over Composio
    const seenKeys = new Set<string>();
    const allItems: typeof lobehubSkillItems = [];

    // Add ModelNet items first (they take priority)
  ...
  // integrations (Composio/ModelNet Skill gateway plugins with other sources like
  // 'self'/'builtin') don't leak in here and duplicate the brand-icon items
  // already rendered under the ModelNet group.
  ...
  // Build ModelNet group children (including Builtin Agent Skills, builtin tools, and ModelNet Skill/Composio)
  const lobehubGroupChildren: ItemType[] = [
    // 1. Builtin Agent Skills
    ...builtinAgentSkillItems,
    // 2. Builtin tools
    ...builtinItems,
    // 3. ModelNet Skill and Composio (as builtin skills)
    ...skillItems,
  ];
  ...
  // (e.g. a Composio/ModelNet integration item plus an installed plugin sharing
  // the same identifier), which would otherwise render the row twice. Keep the
  // first occurrence so the richer integration item (ModelNet group, listed
  ...
  const installedPluginItems: ItemType[] = useMemo(() => {
    ...
    // Connected ModelNet Skill Providers
    const connectedLobehubSkillItems = lobehubSkillItems.filter((item) =>
      checked.includes(item.key as string),
    );

    // Merge enabled ModelNet Skill and Composio (as builtin skills)
    ...
    // Build builtin tools group children (including Builtin Agent Skills, builtin tools, and ModelNet Skill/Composio)
