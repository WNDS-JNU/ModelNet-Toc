...
/** Thrown when connecting a ModelNet connector first needs Market auth. */
export class ConnectorConnectionMarketAuthRequiredError extends Error {
  constructor() {
    super('Market auth required before connecting ModelNet connector');
...
export const useConnectorConnection = (
  ...
  // Only ModelNet connector OAuth signals completion via postMessage; Composio relies on polling.
  useEffect(() => {
