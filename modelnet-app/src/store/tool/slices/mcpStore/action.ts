...
const buildCloudMcpManifest = (params: {
  ...
  // Get tools (MCP format) or api (ModelNet format) from data
  const mcpTools = data.tools;
  const lobeChatApi = data.api;

  // If MCP format tools, need to convert to ModelNet api format
  // MCP: { name, description, inputSchema }
  // ModelNet: { name, description, parameters }
  let apiArray: any[] = [];

  if (lobeChatApi) {
    // Already in ModelNet format, use directly
    apiArray = lobeChatApi;
    log('[Cloud MCP] Using existing ModelNet API format');
  } else if (mcpTools && Array.isArray(mcpTools)) {
    // Convert MCP tools format to ModelNet api format
    apiArray = mcpTools.map((tool: any) => ({
      description: tool.description || '',
      name: tool.name,
      parameters: tool.inputSchema || {},
    }));
    log('[Cloud MCP] Converted %d MCP tools to ModelNet API format', apiArray.length);
  } else {
...
export class PluginMCPStoreActionImpl {
  ...
  installMCPPlugin = async (
    ...
    let connection: any;
    const userAgent = `ModelNet Desktop/${CURRENT_VERSION}`;
