...
/**
 * ModelNet Skill Store Selectors
 */
export const lobehubSkillStoreSelectors = {
  /**
   * Get all ModelNet Skill server identifiers as a set
   */
  ...
  /**
   * Get all ModelNet Skill servers
   */
  getServers: (s: ToolStoreState): LobehubSkillServer[] => s.lobehubSkillServers || [],

  /**
   * Check if the given identifier is a ModelNet Skill server
   * @param identifier - Provider identifier (e.g., 'linear')
   */
  ...
  /**
   * Get all ModelNet Skill tools as LobeTool format for agent use
   * Converts ModelNet Skill tools into the format expected by ToolNameResolver
   */
      ...
      if (apis.length > 0) {
        tools.push({
          ...
          manifest: {
            ...
            meta: {
              avatar: server.icon || '🔗',
              description: `ModelNet Skill: ${server.name}`,
  ...
  /**
   * Get metadata list for all connected ModelNet Skill servers
