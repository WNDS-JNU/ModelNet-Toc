...
const getRenderDisplayControl =
  ...
  (s: ToolStoreState): RenderDisplayControl => {
    ...
    // Fallback for packages that don't ship a ModelNet manifest (e.g. Claude Code —
 ...
 * 2. User-installed plugins (from s.installedPlugins) — exclude Composio/ModelNet Skill/agent skill overlap
 * 3. Composio MCP servers (connected) — description from COMPOSIO_APP_TYPES
 * 4. ModelNet Skill servers (connected) — description from LOBEHUB_SKILL_PROVIDERS
 */
const availableToolsForDiscovery = (s: ToolStoreState): AvailableToolForDiscovery[] => {
  ...
  //    Exclude Composio, ModelNet Skill, and agent skill entries (they are handled in dedicated sources)
  ...
  const composioItems = (s.composioServers || [])
    ...
    // A connector identifier can have legacy Composio and current ModelNet
    // connections at the same time. Tool discovery is identifier-based, so the
    // canonical ModelNet connection must own the single visible entry.
    .filter(
      (server) =>
  ...
  // 4. ModelNet Skill servers (connected only)
