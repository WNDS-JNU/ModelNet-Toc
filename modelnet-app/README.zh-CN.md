...
ModelNet 帮你把专属 Agent 组织成 7×24 不打烊的高效队伍：
...
ModelNet 是一个工作与生活空间，用于发现、构建并与会随着您一起成长的 Agent 队友协作。在 ModelNet 中，我们将 **Agent 视为工作单元**，提供一个让人类与 Agent 共同进化的基础设施。
...
ModelNet 引入了 **Agent Groups**，让您可以像对待真实队友一样与 Agent 协同工作。系统会为任务组装合适的 Agent，支持并行协作与迭代改进。
...
最好的 AI 是能深入理解您的那一种。ModelNet 提供了构建清晰用户理解的 **个人记忆（Personal Memory）**。
...
ModelNet 提供了 Vercel 的 自托管版本 和 [Docker 镜像][docker-release-link]，这使你可以在几分钟内构建自己的聊天机器人，无需任何基础知识。

> \[!TIP]
>
> 完整教程请查阅 [📘 构建属于自己的 ModelNet][docs-self-hosting]
...
我们提供了一个用于在您自己的私有设备上部署 ModelNet 服务的 Docker 镜像。请使用以下命令启动 ModelNet 服务：
...
API Key 是使用 ModelNet 进行大语言模型会话的必要信息，本节以 OpenAI 模型服务商为例，简要介绍获取 API Key 的方式。
...
- 将此 API Key 填写到 ModelNet 的 API Key 配置中，即可开始使用。
...
| [@lobehub/lint][lobe-lint-link]   | [lobehub/lobe-lint][lobe-lint-github]   | ModelNet 代码样式规范 ESlint，Stylelint，Commitlint，Prettier，Remark 和 Semantic Release | [![][lobe-lint-shield]][lobe-lint-link]   |
...
插件提供了扩展 ModelNet [Function Calling][docs-function-call] 能力的方法。可以用于引入新的 Function Calling，甚至是新的消息结果渲染方式。如果你对插件开发感兴趣，请在 Wiki 中查阅我们的 [📘 插件开发指引][docs-plugin-dev] 。
...
- [@lobehub/chat-plugin-sdk][chat-plugin-sdk]：插件 SDK 可帮助您创建出色的 ModelNet 插件。
- [@lobehub/chat-plugins-gateway][chat-plugins-gateway]：插件网关是一个后端服务，作为 ModelNet 插件的网关。我们使用 Vercel 部署此服务。主要的 API POST /api/v1/runner 被部署为 Edge Function。
...
> 同时欢迎联系我们提供产品功能和使用体验反馈，帮助我们将 ModelNet 建设得更好。
>
...
Copyright © 2025 [ModelNet][profile-link]. <br />
