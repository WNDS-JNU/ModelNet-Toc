...
/**
 * Replace the hardcoded ModelNet wordmark in the static loading screen with the
 * custom brand name, so white-label deployments (BRANDING_NAME !== 'LobeHub')
 * never flash the ModelNet logo before the SPA boots. No-op for the default
 * branding.
 */
export const customBrandingLoadingScreen = (): Plugin => ({
  ...
  transformIndexHtml: {
    handler(html) {
      if (BRANDING_NAME === 'ModelNet') return html;
