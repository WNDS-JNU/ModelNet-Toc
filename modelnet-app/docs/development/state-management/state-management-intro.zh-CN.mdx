---
title: 状态管理最佳实践
description: 探索 ModelNet 中的状态管理最佳实践，提升数据流架构的易用性与维护性。
tags:
  - 状态管理
  - ModelNet
  - 数据流
...
ModelNet 不同于传统 CRUD 的网页，存在大量的富交互能力，如何设计一个易于开发与易于维护的数据流架构非常重要。本篇文档将介绍 ModelNet 中的数据流管理最佳实践。
...
### ModelNet SessionStore 目录结构最佳实践

在 ModelNet 应用中，由于会话管理是一个复杂的功能模块，因此我们采用了 [slice 模式](https://github.com/pmndrs/zustand/blob/main/docs/guides/slices-pattern.md) 来组织数据流。下面是 ModelNet SessionStore 的目录结构，其中每个目录和文件都有其特定的用途：
...
在 ModelNet 中，SessionStore 被设计为管理会话状态和逻辑的核心模块。它由多个 Slices 组成，每个 Slice 管理一部分相关的状态和逻辑。下面是一个简化的 SessionStore 的实现示例：
...
在这个 `store.ts` 文件中，我们创建了一个 `useSessionStore` 钩子，它使用 `zustand` 库来创建一个全局状态管理器。我们通过共享的 `flattenActions` 工具函数，将 initialState 和每个 Slice 的动作合并，以创建完整的 SessionStore。这里没有使用 `persist` 中间件：SessionStore 的数据通过 SWR 从服务端获取，而不是持久化到本地存储。
...
在 `action.ts` 文件中，我们定义了一个 `SessionAction` 接口来描述会话相关的动作，并且实现了一个 `useFetchSessions` 函数来创建这些动作。然后，我们将这些动作与初始状态合并，以形成会话相关的 Slice。

通过这种结构分层和模块化的方法，我们可以确保 ModelNet 的 SessionStore 是清晰、可维护的，同时也便于扩展和测试。
