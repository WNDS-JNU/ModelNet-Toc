---
title: ModelNet Feature Development Complete Guide
description: A comprehensive guide for developers on implementing features in ModelNet.
tags:
  - ModelNet
  - Feature Development
  - Developer Guide
...
# ModelNet Feature Development Complete Guide

This document aims to guide developers on how to develop a complete feature in ModelNet.
...
ModelNet uses a PostgreSQL database, with [Drizzle ORM](https://orm.drizzle.team/) to operate the database.
...
| tRPC Router       | apps/server/src/routers/             | tRPC entry, validates input, routes to service        |
| Server Service    | apps/server/src/services/            | Server-side business logic, with DB access            |
| Server Module     | apps/server/src/modules/             | Server-side modules, no direct DB access              |
...
**tRPC Router** is the backend entry point (`apps/server/src/routers/lambda/`), validates input and calls Server Service for business logic:
...
ModelNet uses [zustand](https://zustand.docs.pmnd.rs/getting-started/introduction) as the global state management framework. For detailed practices on state management, refer to [State Management Best Practices](/docs/development/state-management/state-management-intro).
...
First, we update the initialState. After reading `src/features/AgentSetting/store/initialState.ts`, we learn that the initial agent configuration is saved in `DEFAULT_AGENT_CONFIG` in `packages/const/src/settings/agent.ts`:
...
Update `src/store/agent/selectors/selectors.ts`:
...
ModelNet is an internationalized project using [react-i18next](https://github.com/i18next/react-i18next). Newly added UI text needs to:

1. Add keys to the corresponding namespace file in `packages/locales/src/default/` (default language is English):
...
2. If a new namespace is added, export it in `packages/locales/src/default/index.ts`
3. For dev preview: manually translate the corresponding JSON files in `locales/zh-CN/` and `locales/en-US/`
4. CI will automatically run `bun run i18n` to generate translations for other languages
...
We also need to display the opening configuration set by the user on the chat page. Agent chat uses `src/features/AgentHome` (`AgentInfo.tsx` for `openingMessage`, `OpeningQuestions.tsx` for questions). Group chat has a parallel welcome under `src/routes/(main)/group/features/Conversation/AgentWelcome`. Do **not** use `src/features/Onboarding/Agent/WelcomeMessage.tsx` — that is the onboarding flow only.

```typescript
// src/features/AgentHome/AgentInfo.tsx (simplified)
const AgentInfo = () => {
  const { t } = useTranslation(['chat', 'welcome']);

  // Prefer conversation-scoped agent config (openingMessage on agent config)
  ...
  // render avatar + Markdown(message) ...
};

// src/features/AgentHome/index.tsx wires AgentInfo + OpeningQuestions
```
...
The above is the complete implementation process for the ModelNet opening settings feature, covering the full chain from database schema → data model → Service/Model → Store → i18n → UI → testing. Developers can refer to this document for developing related features.
