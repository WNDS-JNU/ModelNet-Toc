---
title: Configuring Microsoft Authentication for ModelNet
description: >-
  Learn how to configure Microsoft (Entra ID / Azure AD) SSO for ModelNet,
...
<Steps>
  ...
  <Callout type={'warning'}>
    **Deprecated Environment Variables**: The following legacy variable names
    are no longer supported. Setting any of them will cause the deployment to
    fail at build/startup — use `AUTH_MICROSOFT_ID` / `AUTH_MICROSOFT_SECRET` /
    `AUTH_MICROSOFT_TENANT_ID` instead:
