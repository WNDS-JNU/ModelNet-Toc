---
title: Configuring Keycloak Authentication Service in ModelNet
description: >-
  Learn how to configure the Keycloak authentication service in ModelNet,
...
# Configuring Keycloak Authentication Service in ModelNet

<Callout type={'warning'}>
  NextAuth (Auth.js) has been fully removed from ModelNet. The `NEXT_AUTH_SSO_PROVIDERS` and `AUTH_URL` variables shown below will now cause the deployment to fail at build/startup. This page is kept for historical reference only — use the [Better Auth provider guides](/docs/self-hosting/auth) and the [NextAuth to Better Auth migration guide](/docs/self-hosting/migration/v2/auth/nextauth-to-betterauth) instead.
</Callout>
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with ModelNet via Docker
...
- Your ModelNet database version IP/port is `http://LobeHub_IP:3210`.
- Your privately deployed Keycloak domain is `http://KEYCLOAK_IP:8080`.

If you deploy using a public network, this guide assumes:

- Your ModelNet database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "ModelNet", then click "Create"
  ...
  2. Create a Client
     ...
     - On the "Login settings" page:
       ...
       - Web origins: Add your ModelNet domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the ModelNet environment variables.

  Configure the ModelNet environment variable `AUTH_KEYCLOAK_ISSUER` as:

  - `http://localhost:8080/realms/ModelNet` for local development environment
  - `http://KEYCLOAK_IP:8080/realms/ModelNet` for privately deployed Keycloak on a local network
  - `https://lobe-auth-api.example.com/realms/ModelNet` for Keycloak deployed in a public environment

  When deploying ModelNet, you need to configure the following environment variables:
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for ModelNet. For Keycloak, fill in `keycloak`.                                                                                                        |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate through Keycloak and use ModelNet.
</Callout>
