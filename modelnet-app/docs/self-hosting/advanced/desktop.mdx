---
title: Automatic Synchronization with ModelNet Desktop
description: Configure ModelNet Desktop for a synchronized experience
tags:
  - ModelNet
...
# Automatic Synchronization with ModelNet Desktop

ModelNet Desktop provides users with an enhanced experience while also expanding the capabilities of ModelNet, allowing users to utilize ModelNet in offline environments. This document will guide you on how to configure ModelNet Desktop and connect it to your self-hosted instance.

## System Requirements

ModelNet Desktop is built using Electron and supports the following operating systems:
...
The version release of ModelNet Desktop follows this logic:

- **Beta Version**: All merges to the main branch that are automatically released will be created as Beta versions of ModelNet Desktop.
...
ModelNet Desktop can connect to your self-hosted ModelNet instance, allowing you to use your self-hosted configuration on the desktop.
...
OIDC is enabled automatically once the `JWKS_KEY` environment variable is configured on your self-hosted instance — there is no separate `ENABLE_OIDC` switch to set.

Click the button below to generate `JWKS_KEY`:

<GenerateJWKSKey />

Add the generated environment variable to your deployment configuration.

<Callout>`OIDC_JWKS_KEY` has been renamed to `JWKS_KEY`. If your deployment still has `OIDC_JWKS_KEY` set, you need to manually rename it to `JWKS_KEY` — there is no automatic fallback.</Callout>

If you are deploying ModelNet using one-click deployment methods (such as Vercel, Railway, etc.), you need to:
...
1. Open ModelNet Desktop.
2. Click the Wi-Fi icon in the upper right corner.
3. Enter the address of your self-hosted instance (e.g., `https://your-ModelNet-instance.com`).
...
- Confirm that you have correctly set the `JWKS_KEY` environment variable.
...
- Example file names:
  - Apple Silicon: `ModelNet-Beta-1.83.6-arm64.dmg` or `ModelNet-Beta-1.83.6-arm64-mac.zip`
  - Intel: `ModelNet-Beta-1.83.6-x64.dmg` or `ModelNet-Beta-1.83.6-mac.zip`
