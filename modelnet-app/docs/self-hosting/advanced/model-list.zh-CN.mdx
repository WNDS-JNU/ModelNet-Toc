---
title: ModelNet 自定义模型服务商模型列表及扩展能力配置
description: 了解如何在 ModelNet 中自定义模型列表以及扩展能力配置的基本语法和规则。
tags:
  - ModelNet
  - 自定义模型列表
  - 扩展能力配置
...
ModelNet 支持在部署时自定义模型列表，详情请参考 [模型提供商](/zh/docs/self-hosting/environment-variables/model-provider) 。
...
部署名`->deploymentName`可以省略，默认为最新版本的模型。当前支持`->deploymentName`的模型服务商有：Azure、Azure AI、Qwen、Spark、Volcengine（及其 coding plan）以及 Kimi Coding Plan。

例如： `+qwen-7b-chat,+glm-6b,-gpt-3.5-turbo,gpt-4-turbo=gpt-4o`
...
仅启用 gpt-3.5-turbo 和 gpt-4-turbo（展示名为 gpt-4o），而其他模型都隐藏。
...
- `gemini-2.5-flash=Gemini 2.5 Flash<16000:vision>`：Google 视觉模型，最大上下文 16k，支持图像识别；
...
- 确认该模型在提供商 API 中实际支持该能力（ModelNet 无法开启 API 本身不提供的能力）
- 确认你运行的是足够新的 ModelNet 版本
