...
description: >-
  Learn how to configure Redis cache service to optimize ModelNet performance and
  session management.
tags:
  - Redis
  - Cache
...
ModelNet uses Redis as a high-performance cache and session storage service to optimize system performance and manage user authentication state.

<Callout type={'info'}>
  ModelNet uses the standard Redis protocol (via ioredis library), supporting any Redis
...
Redis is used in ModelNet for the following scenarios:
...
<Steps>
  ...
  The prefix for Redis keys, used to isolate ModelNet data in a shared Redis instance.
...
<Callout type={'warning'}>
  Redis is an optional service. If `REDIS_URL` is not configured, ModelNet will still function
