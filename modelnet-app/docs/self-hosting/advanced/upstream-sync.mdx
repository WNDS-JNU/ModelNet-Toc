---
title: ModelNet Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for ModelNet Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest ModelNet image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running ModelNet Container

  Assuming the ModelNet container is named `lobehub`, use the following commands to stop and remove the currently running ModelNet container:
  ...
  ### Pull the Latest ModelNet Image

  Use the following command to pull the latest Docker image for ModelNet:
  ...
  Redeploy the ModelNet container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of ModelNet's chat records are stored in your local browser. Therefore, when redeploying ModelNet using Docker, your chat records will not be lost.
</Callout>
