---
title: Deploying ModelNet Database on Zeabur
description: >-
  Learn how to deploy ModelNet on Zeabur with ease. Follow the provided steps to
...
# Deploying ModelNet Database on Zeabur
...
Here is the process for deploying the ModelNet server database version on Zeabur:

<Steps>
  ### Go to the template page on Zeabur

  Go to the [ModelNet Database template page](https://zeabur.com/templates/RRSPSD) on Zeabur and click on the "Deploy" button.
  ...
  - ModelNet Domain: A free subdomain with `.zeabur.app` suffix.
  ...
  After you fill all the required environment variables, select a region where you want to deploy your ModelNet Database and click on the "Deploy" button.
  ...
  Fill in those variables into your ModelNet service on Zeabur, here is a more detailed guide for [editing environment variables on Zeabur](https://zeabur.com/docs/deploy/variables).

  For detailed configuration of Logto, refer to [this document](/docs/self-hosting/auth/providers/logto).

  ### Access your ModelNet Instance

  Press on the `ModelNet-Database` and you can see the public domain you just created, click on it to access your ModelNet Database.
