---
title: Deploying ModelNet Database with Docker
description: >-
  Learn how to deploy the ModelNet server database version using Docker on Linux
  and local machines.
tags:
  - ModelNet
  - Docker
...
Here is the process for deploying the ModelNet server database version on a Linux server:
...
<Steps>
  ...
  docker run --name my-postgres --network pg -e POSTGRES_PASSWORD=mysecretpassword -p 5432:5432 -d paradedb/paradedb:latest-pg17 -c shared_preload_libraries=pg_search
  ```

  The above command will create a Postgres instance named `my-postgres` with the network set to `pg`, where `paradedb/paradedb:latest-pg17` is a Postgres 17 image with the `pgvector` and `pg_search` extensions installed and enabled by default.

  <Callout type="info">
    The ParadeDB image includes pgvector (vector search) and pg\_search (full-text search) plugins, which are important
    components for ModelNet to implement RAG and knowledge base search.

    It should be noted that the new version of ParadeDB requires explicitly enabling the pg_search extension in order to be used by ModelNet. Here we explicitly enable pg_search.
  </Callout>
...
The data version of ModelNet also supports direct use on a local Mac/Windows machine.
