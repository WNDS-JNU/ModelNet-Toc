...
From June through July, ModelNet focused on two things: making tools easier to discover and making search more flexible. The new MCP Marketplace brings one-click plugin installation to desktop, while expanded search providers and authentication options give teams more choice in how they work.
...
On the authentication side, Amazon Cognito and Google SSO are now available as identity providers. Teams already using these systems can connect ModelNet without adding another set of credentials. Page access protection is also included, so you can control who sees what based on their sign-in status.
