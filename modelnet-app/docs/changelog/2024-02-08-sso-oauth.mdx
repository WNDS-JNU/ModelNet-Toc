...
description: >-
  ModelNet now supports both Clerk and Next-Auth, giving teams flexibility to
...
Every product needs reliable sign-in, but not every team has the same requirements. Some need to get up and running quickly with social logins. Others need enterprise-grade controls from day one. ModelNet now supports both paths by integrating with next-auth and Clerk.
