---
title: Connect ModelNet to WeChat
description: >-
  Learn how to connect a WeChat bot to your ModelNet agent via the iLink Bot API,
...
# Connect ModelNet to WeChat

By connecting a WeChat channel to your ModelNet agent, users can interact with the AI assistant through WeChat private chats and group conversations.
...
In ModelNet, navigate to your agent's settings, then select the **Channels** tab. Click **WeChat** from the platform list.
...
<Steps>
  ...
  Open WeChat on your phone, go to **Scan** (via the + button in the top right), and scan the QR code displayed in ModelNet.
  ...
  Once confirmed, ModelNet will automatically save your credentials and connect the bot. You should see a success message in the channel settings.
</Steps>

## Step 3: Test the Bot

Open WeChat, find your bot contact, and send a message. The bot should respond through your ModelNet agent.
...
1. When you scan the QR code, ModelNet obtains a bot token from WeChat's iLink API
2. ModelNet continuously polls the iLink API for new messages (\~35 second intervals)
3. When a message arrives, it is routed through the ModelNet agent for processing
