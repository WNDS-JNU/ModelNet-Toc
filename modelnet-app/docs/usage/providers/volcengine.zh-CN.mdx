---
title: 在 ModelNet 中使用火山引擎 API Key
description: 学习如何在 ModelNet 中配置和使用火山引擎 AI 模型，获取 API 密钥并开始对话。
tags:
  - ModelNet
  - 火山引擎
  - 豆包
...
<Image alt={'在 ModelNet 中使用火山引擎'} cover src={'/blog/assets80a8b9627374fc345f4bf8e3adf11074.webp'} />

[火山引擎](https://www.volcengine.com/)是字节跳动旗下的云服务平台，通过 "火山方舟" 提供大型语言模型 (LLM) 服务，支持多个主流模型如豆包（Doubao）、DeepSeek、GLM 等。

本文档将指导你如何在 ModelNet 中使用火山引擎:
...
<Steps>
  ...
  ### 步骤二：在 ModelNet 中配置火山引擎

  - 访问 ModelNet 的 `应用设置` 的 `AI 服务供应商` 界面
  - 在供应商列表中找到 `火山引擎` 的设置项
...
至此你已经可以在 ModelNet 中使用火山引擎提供的模型进行对话了。
