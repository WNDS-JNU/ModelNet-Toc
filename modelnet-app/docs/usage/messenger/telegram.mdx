---
title: Use ModelNet on Telegram
description: >-
  Link your ModelNet account to the official ModelNet bot on Telegram. Send /start
...
# Use ModelNet on Telegram

Telegram is the simplest Messenger platform: there is one global ModelNet bot, and any Telegram account can DM it. You only need to link your ModelNet account once.
...
In ModelNet, open **Settings → Messenger** and click the **Telegram** card.
...
Either tap **Open in Telegram** in the modal, or scan the QR code with your phone. Telegram opens the official ModelNet bot.
...
Tap the link in the bot's reply. You'll be taken back to ModelNet. Sign in if asked, then choose a **default agent** — every message you DM the bot from now on will route to this agent.

{/* TODO: screenshot — confirm-link page in ModelNet with the agent picker visible */}
...
- **In ModelNet** — open **Settings → Messenger → Telegram** and use the agent picker on your link row.
...
> Disconnecting from ModelNet does not remove the bot from your Telegram chat list — you can manually delete the chat on Telegram if you no longer want to see it.
...
- **"This account is already linked"** — Your Telegram account is bound to a different ModelNet account. Sign in to that ModelNet account to manage the link, or unlink there first.
- **"Another Telegram account is already linked"** — Your ModelNet account already has a Telegram link. Disconnect it in **Settings → Messenger → Telegram** before linking a new Telegram account.
