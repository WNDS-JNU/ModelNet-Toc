...
> 状态：实施中（PR 1–5 Dev 代码与部署完成；single / broadcast / parallel_tasks、核心恢复故障注入、显式重试及乱序回调通过，待暂停验收）
> 版本：V2.0
> 重构日期：2026-08-30
