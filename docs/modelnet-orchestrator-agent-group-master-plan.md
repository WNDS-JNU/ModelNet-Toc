...
> 状态：实施中（PR 1–5 Dev 代码与部署完成；single / broadcast / parallel_tasks 及核心恢复故障注入通过，待暂停、显式重试与乱序回调验收）
> 版本：V2.0
> 重构日期：2026-08-30
