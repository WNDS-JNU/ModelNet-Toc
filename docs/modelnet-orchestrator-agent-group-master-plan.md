...
> 状态：实施中（PR 1–5 Dev 代码与部署完成；single / broadcast / parallel_tasks、核心恢复故障注入及显式重试通过，待暂停与乱序回调验收）
> 版本：V2.0
> 重构日期：2026-08-30
