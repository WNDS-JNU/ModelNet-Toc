...
LobeHub / SDK / 客户端
  -> LiteLLM
  -> modelnet-router
  -> vLLM / llama.cpp / OpenAI-compatible / Ollama 等后端
```

`modelnet-router` 是这份文档的重点。它在 Docker Compose 里暴露内部 8000 端口，默认宿主机端口为 `127.0.0.1:3092`。LiteLLM 容器通过 `http://modelnet-router:8000/v1` 调用它，并对外提供 `127.0.0.1:3090`。
...
- `litellm/modelnet-config.yaml`：LiteLLM 到 modelnet-router 的代理配置。
